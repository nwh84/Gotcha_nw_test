...
MutationCalling = function(out = "/path_to_filtered_fastqs/",
  ...
  test <- Primed.output.test[[1]]
  Matched.output = lapply(Primed.output, function(y){
    # check if the barcodes have a perfect match
    ind_match <- (y$CB %in% whitelist)
    y$WhiteListMatch[ind_match] <- y$CB[ind_match]
    # apply hamming distance for barcodes that don't have perfect match
    y$WhiteListMatch[!ind_match] = lapply(y$CB[!ind_match], function(x){
